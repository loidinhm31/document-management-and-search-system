...
class AdminService extends BaseService {
  ...
  getUser(id: string) {
    return this.handleApiResponse<UserData>(axiosInstance.get(`/v1/users/${id}`));
  }

  getAllRoles() {
    return this.handleApiResponse<Role[]>(axiosInstance.get("/v1/admin/roles"));
  }

  updateUserRole(userId: string, request: { userId: string; roleName: string }) {
    return this.handleApiResponse(axiosInstance.put(`/v1/users/${userId}/role`, request), {
  ...
  updateStatus(userId: string, request: UpdateStatusRequest) {
    return this.handleApiResponse(axiosInstance.put(`/v1/users/${userId}/status`, request), {
  ...
  getStats() {
    return this.handleApiResponse(axiosInstance.get("/v1/admin/stats"));
  }
  ...
  getAuditLogs(
    ...
    return this.handleApiResponse(axiosInstance.get("/v1/admin/audit-logs", { params }));
  }
}
